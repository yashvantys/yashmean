...
export class ContentComponent implements OnInit {
  ...
  contentList() {
    this.dtOptions = {
      ...
      searching: true,
      "language": {
        "emptyTable": "No data available in Todo List",
      ...
      columns: [
        { data: "title" },
        {
          data: null, render: function (data, type, row) {
            var html
            html = `<button type="button" class="btn btn-primary editbutton" contents-id="${data._id}" contents-title="${data.title}" contents-description="${btoa(data.description)}">Edit</button>`
            html += `&nbsp;<button type="button" class="btn btn-danger deletebutton" contents-id="${data._id}">Delete</button>`;
            ...
            this.contentsservice.deleteContent(contentId, flag)
              .subscribe(res => {
                if (res['statusCode'] == 200) {
                  $('#ajaxResults').addClass('alert alert-success').html('Todo ' + msg + ' successfully');
                  setTimeout(function () {
      ...
      this.contentsservice.updateContent(this.contentsForm.value, contentId)
        .subscribe(response => {
          if (response['statusCode'] == 200) {
            ...
            $('#ajaxResults').addClass('alert alert-success').html('Todo updated successfully')
            setTimeout(function () {
              $('#ajaxResults').removeClass('alert alert-success').html('')
            }, 2000);
            var table = $('#contents-list').DataTable()
        ...
        .subscribe(
          response => {
            if (response['statusCode'] == 200) {
              this.contentsForm.reset();
              (<any>$('#content-add')).modal('hide')
              $('#ajaxResults').addClass('alert alert-success').html('Todo added successfully')
              setTimeout(function () {
                $('#ajaxResults').removeClass('alert alert-success').html('')
              }, 2000);
              var table = $('#contents-list').DataTable()
